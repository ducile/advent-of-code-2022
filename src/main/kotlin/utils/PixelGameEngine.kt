 ...
 * # PixelGameEngine
 *
 * An absolutely simple and rudimentary engine to quickly draw rough, pixelized things on screen. Suitable for demos,
 * little games, mazes, etc.
 *
 * It displays a Swing window of defined size and runs in an endless game loop, allowing your code to act
 ...
 *
 * Start using [construct] method and then start game loop using [start].
 *
 */
abstract class PixelGameEngine {
    ...
    fun start() {
        ...
        while (!halted && isActive()) {
            var time = measureTimeMillis {
                halted = !onUpdate(System.currentTimeMillis() - startTime, frame++)
                displayBuffer = buffer
     ...
     *
     * Must return true to keep on being called in a loop.
     * @param elapsedTime the elapsed time in ms since start of the app
     * @param frame the number of the current frame
     * @return true to keep on running the update loop, false to stop the visualization
     * @see draw
     * @see drawLine
     * @see drawRect
     * @see fillRect
     ...
     * @see sleep
     */
    open fun onUpdate(elapsedTime: Long, frame: Long): Boolean {
        return false
    }
